...
async function writeSingleTvl(isLp = false, vid, vaultInfo, token) {
    ...
    if (!isLp) {
        ...
        axios.get(url)
            .then(response => {
                ...
                let rewardTotal = priceToken1 * rewardAmount
            ...
            .then(response => {
                ...
                axios.get(url)
                    .then(response => {
                        ...
                        let total = ((rewardAmount * price) + (usersAmount * price)).toString();

                        console.log(usersAmount, rewardAmount)
...
async function writeMultiTvl(isLP = false, vid, vaultInfo, stake_token, reward_token) {
    ...
    if (!isLP) {
        ...
        if (stakeContractIsLP && rewardContractIsLP) {
            ...
            console.log('ready for some reqs')
                                ...
                                .then(response => {
                                    ...
                                    axios.get(urlTokenReward1)
                                        .then(response => {
                                            ...
                                            let tvl = (totalStake + totalReward).toString()
                                            addTVL(vid, tvl)
                        ...
                        .then(response => {
                            ...
                            axios.get(urlTokenStake)
                                .then(response => {
                                    ...
                                    let total = (totalReward + stakeTotal).toString()
...
async function addTVL(vid, total) {
    var sql = `select *
               from vaults_rewards_value
               where vid = ?`

    let params = [moment().unix(), total.toString(), vid]
